...
| Nghiệm thu clip | `vslr-check --data-dir dataset/recordings_v1` — dùng recording plan, báo clip cần quay lại/cặp còn thiếu. Không train |
| Train | `vslr-train --data-dir dataset/recordings_v1` — plan gate rồi train mọi clip, **không holdout, không kèm accuracy** |
| Đo accuracy | `vslr-train --data-dir dataset/recordings_v1 --loso` — ghi `models/loso_report.json`, **không** ghi checkpoint |
| Train một người | `vslr-train --video "Nhãn=path.mov" ...` (≥ 2 clip mỗi nhãn; `--loso` bị từ chối vì không có ID người) |
| Camera | `vslr-camera --no-tts` — checkpoint lệch feature contract/dimension bị chặn, không có bypass giả an toàn |
...
| `dataset/recording_plan.json` | hợp đồng V1 P01–P04 × 6 clip/cặp; init/check/train directory mode cùng đọc | tracked |
