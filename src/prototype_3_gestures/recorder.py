...
def resolve_record_dir(record_dir: str | Path | None = None) -> Path:
    """Xác định thư mục lưu trữ video cử chỉ."""
    if record_dir is not None:
        p = Path(record_dir).resolve()
        p.mkdir(parents=True, exist_ok=True)
        return p

    # Đường dẫn thư mục chuẩn của dự án theo yêu cầu
    project_explicit = Path(r"D:\1. Nguyễn Trường Thọ\NCKH\VSLR-Project-pipeline-signer-split\videos")
    if project_explicit.parent.exists():
        project_explicit.mkdir(parents=True, exist_ok=True)
        return project_explicit

    # Tìm kiếm theo thư mục tương đối
    cwd = Path.cwd()
    if (cwd / "videos").is_dir() or (cwd / "videos").parent.exists():
        target = cwd / "videos"
    elif (cwd.parent / "videos").is_dir() or (cwd.parent / "videos").parent.exists():
        target = cwd.parent / "videos"
    else:
        target = cwd / "videos"
        ...
        if results is not None:
            if hasattr(results, "pose_landmarks") and results.pose_landmarks:
                self._mp_drawing.draw_landmarks(
                    ...
                    landmark_drawing_spec=self._mp_drawing.DrawingSpec(color=(60, 180, 75), thickness=2, circle_radius=2),
                    connection_drawing_spec=self._mp_drawing.DrawingSpec(color=(255, 225, 25), thickness=2, circle_radius=2),
                )
