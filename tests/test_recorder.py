...
class TestRecorder(TestCase):
    ...
    def test_draw_body_pose_landmarks_excludes_face_points(self):
        # 1. Khẳng định toàn bộ kết nối thân người đều bắt đầu từ khớp vai (>= 11), không có điểm mặt (0-10)
        self.assertGreater(len(POSE_BODY_CONNECTIONS), 0)
        for start, end in POSE_BODY_CONNECTIONS:
            self.assertGreaterEqual(start, 11, f"Kết nối có điểm xuất phát thuộc mặt: {start}")
            self.assertGreaterEqual(end, 11, f"Kết nối có điểm đích thuộc mặt: {end}")
        ...
        class FakePoseLandmarks:
            def __init__(self, count):
                # Tạo 33 điểm
                self.landmark = [FakeLandmark(0.5, 0.5) for _ in range(count)]

        # 2. Tạo landmark chỉ gồm các điểm mặt (chỉ số 0-10 đặt toạ độ hợp lệ, các điểm thân đặt ngoài vùng / visibility 0)
        face_only = FakePoseLandmarks(33)
        for idx in range(11, 33):
            face_only.landmark[idx].visibility = 0.0

        canvas_face = np.zeros((100, 100, 3), dtype=np.uint8)
        draw_body_pose_landmarks(canvas_face, face_only)
        # Các điểm mặt 0-10 hoàn toàn bị bỏ qua, không có pixel nào bị vẽ lên
        self.assertEqual(int(np.sum(canvas_face)), 0, "Các điểm mặt (0-10) không được phép vẽ lên canvas!")

        # 3. Tạo landmark có các điểm thân (vai, khuỷu tay 11-14)
        body_pose = FakePoseLandmarks(33)
        body_pose.landmark[11].x, body_pose.landmark[11].y = 0.4, 0.4
        body_pose.landmark[12].x, body_pose.landmark[12].y = 0.6, 0.4
        canvas_body = np.zeros((100, 100, 3), dtype=np.uint8)
        draw_body_pose_landmarks(canvas_body, body_pose)
        # Điểm thân phải được vẽ
        self.assertGreater(int(np.sum(canvas_body)), 0, "Các điểm thân người (>= 11) phải được vẽ lên canvas!")
