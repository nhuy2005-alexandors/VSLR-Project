...
if exist "%CD%\venv\Scripts\python.exe" (
    "%CD%\venv\Scripts\python.exe" -m prototype_3_gestures.web_server --model "%MODEL_PATH%" --camera 0 --allow-uncalibrated --confidence 0.72 --cooldown 1.5 --tts-voice "Trúc Ly" --record-dir "%RECORD_DIR%" --web-dir "%WEB_DIR%" --port 8000
) else if exist "%LOCALAPPDATA%\Programs\Python\Python311\python.exe" (
    "%LOCALAPPDATA%\Programs\Python\Python311\python.exe" -m prototype_3_gestures.web_server --model "%MODEL_PATH%" --camera 0 --allow-uncalibrated --confidence 0.72 --cooldown 1.5 --tts-voice "Trúc Ly" --record-dir "%RECORD_DIR%" --web-dir "%WEB_DIR%" --port 8000
) else (
    python -m prototype_3_gestures.web_server --model "%MODEL_PATH%" --camera 0 --allow-uncalibrated --confidence 0.72 --cooldown 1.5 --tts-voice "Trúc Ly" --record-dir "%RECORD_DIR%" --web-dir "%WEB_DIR%" --port 8000
)
